# 🌐 Nexio: Remote State Management with S3

_Taking Nexio from Local-Only to Collaborative_

## 🧭 Introduction

In my previous posts, I covered how I built Nexio from scratch as a [local VCS](/blog/build-your-own-git-in-go), then [optimized storage with content-addressable blobs](/blog/content-addressable-storage-blake3), and most recently [migrated metadata from JSON to SQLite](/blog/json-to-sqlite-2000x-faster). Through all of that, Nexio had one glaring limitation printed right in the README: **"No remote repository support."**
...
1. **Content-addressable storage makes sync easy.** The blob deduplication from the [previous optimization](/blog/content-addressable-storage-blake3) wasn't just about saving disk space — it made remote sync almost trivial. Same hash means same content, so a `HeadObject` check is all you need to know whether to transfer a blob.
