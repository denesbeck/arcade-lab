# 🏗️ Home Lab: UPS Monitoring & Failover

_Automated shutdown on power loss with Prometheus alerting_
...
The entire setup is captured in an [Ansible playbook](https://github.com/denesbeck/home-lab-infra/blob/main/playbooks/ups-monitor.yml). It creates the textfile collector directory, deploys the power monitor script with all variables templated (the router IP lives in a separate vars file, excluded from version control), sets up the log file, and registers the `@reboot` cron job.
