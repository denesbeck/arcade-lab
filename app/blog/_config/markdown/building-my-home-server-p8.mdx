# 🏗️ Home Lab: Remote Access with Tailscale

_Secure WireGuard-based VPN with Pi-hole DNS and Samba over the mesh_

In my previous blog post, I covered setting up Jellyfin for streaming movies on my local network. In this post, I'm solving a problem that's been bugging me since the beginning: accessing my home lab from outside my local network. Until now, all my services — Jellyfin, Grafana, Pi-hole, Filebrowser, and everything else — were only accessible when I was home on my LAN. If I wanted to stream a movie on my phone while commuting or check my Grafana dashboards from a coffee shop, I was out of luck. Time to fix that.
...
**WireGuard** is the modern alternative — fast, lightweight, and built into the Linux kernel. It's a massive improvement over OpenVPN in terms of performance and simplicity. But you still need to open a port on your router, manage key pairs for every device, and handle DNS configuration yourself.
...
Tailscale provides an official apt repository for Ubuntu. The installation follows the same pattern I used for Docker in Part 4 — download the GPG key, add the repository, and install the package:
...
- **`--advertise-routes=192.168.64.15/32`**: This makes the server a subnet router, advertising its LAN IP to the tailnet. I'll explain why this is needed in the [Subnet Routing](#-subnet-routing) section below.
...
## 🐳 Making Pi-hole Listen on Tailscale

This is where I hit my first snag. After setting everything up, I configured the Tailscale admin console to use my server as a global DNS nameserver (so my phone would use Pi-hole for DNS even when remote). The result? My phone completely lost DNS resolution — I couldn't reach anything, not even public websites.

The problem was straightforward: Pi-hole's DNS port (53) was bound to `192.168.64.15` — the server's LAN IP. When my phone sent DNS queries to the Tailscale IP (`100.104.44.113`), Pi-hole wasn't listening there. Connection refused.
...
I went with the global nameserver because I wanted ad blocking on my phone when I'm out. The tradeoff is that during the daily 3 AM reboot window, DNS briefly breaks on connected Tailscale devices — but I'm usually asleep at that point, so it's an acceptable compromise.

## 🌐 Subnet Routing

With Pi-hole resolving all `*.arcade-lab.io` subdomains to `192.168.64.15` (the LAN IP), remote Tailscale clients need a way to actually reach that IP. By default, Tailscale only routes traffic between devices using their Tailscale IPs (`100.x.y.z`). A remote device has no route to `192.168.64.15` — it's a private IP on my home network.
...
First, the server needs IP forwarding enabled — without it, the kernel won't forward packets between the Tailscale and LAN interfaces:
...
I'm using `/32` instead of `/24` here — there's no reason to expose my entire LAN subnet when all services run on a single host. Principle of least privilege.

### Approving the Route

Advertising a route isn't enough — Tailscale requires explicit approval in the admin console as a safety measure:

1. Go to [Tailscale Machines](https://login.tailscale.com/admin/machines)
2. Find the server node and click **"..."** → **"Edit route settings"**
...
1. Phone queries Pi-hole at `100.104.44.113` → gets back `192.168.64.15`
2. Phone sends HTTPS request to `192.168.64.15`
3. Tailscale routes the traffic to the subnet router (the server)
4. Server forwards the packet to `192.168.64.15` — which is itself
5. The service responds, and the reply follows the same path back

The beauty of this approach is that it keeps the DNS setup simple (one IP per domain), works transparently for both local and remote clients, and doesn't interfere with the public `arcade-lab.io` domain hosted on Vercel — Pi-hole has no record for the root domain, so those queries pass through to upstream DNS.

## 🔒 Access Controls

By default, Tailscale allows all devices on a tailnet to communicate with each other. That's fine when it's just your own devices, but it's good practice to lock things down — especially with subnet routing enabled. Tailscale provides tag-based ACLs (Access Control Lists) that you configure in the [admin console](https://login.tailscale.com/admin/acls).

### Tagging Devices

I created two tags:

- **`tag:server`** — the Ubuntu server running all services
- **`tag:trusted`** — my personal devices (laptop, phone) that should have full access

Tags are assigned to devices in the [Machines](https://login.tailscale.com/admin/machines) page.
...
This allows trusted devices to reach the server via both its Tailscale IP (`100.104.44.113`) and the subnet route (`192.168.64.15`), on all ports. Everything else is denied by default — untagged devices or devices without the `tag:trusted` tag can't reach anything on the tailnet.
...
## 📁 Making Samba Work Over Tailscale
...
The auth key is stored in a variables file that's excluded from version control (like all my other secrets), and the playbook is idempotent — it checks if Tailscale is already authenticated before attempting to re-auth, so running it multiple times doesn't cause issues.

The Tailscale playbook also handles subnet routing — it enables IP forwarding via `sysctl` and passes `--advertise-routes` during authentication. For already-authenticated nodes, it runs `tailscale set` to update the advertised routes without re-authenticating.
...
## 🐛 Cloudflare Wildcard DNS and IPv6

After migrating my domain from Vercel's registrar to Cloudflare, remote access suddenly broke. Accessing `jellyfin.arcade-lab.io` from a Tailscale device returned a Vercel "DEPLOYMENT_NOT_FOUND" error — even though DNS and subnet routing appeared to be working correctly.

The root cause was subtle. When I migrated to Cloudflare, two **proxied wildcard `*` A records** came along for the ride. Even though I never created AAAA (IPv6) records, Cloudflare's proxy automatically injects IPv6 responses for any proxied record. So when a client queried `jellyfin.arcade-lab.io`:

- **A (IPv4)**: Pi-hole responded with `192.168.64.15` — correct
- **AAAA (IPv6)**: Pi-hole had no local answer, so it forwarded the query upstream. Cloudflare's proxy returned its own IPv6 addresses (`2a06:98c1:...`) — pointing to Vercel

Most modern clients prefer IPv6 over IPv4. So despite Pi-hole returning the correct IPv4 address, the client connected to Cloudflare's IPv6 address instead, which routed to Vercel — a deployment that no longer existed.

The fix was simple: **delete the proxied wildcard DNS records in Cloudflare**. These subdomains should only resolve via Pi-hole, not through public DNS. After removing the wildcard records and flushing DNS caches, everything worked as expected.

The takeaway: if you're using Pi-hole for local DNS with only A records, watch out for Cloudflare's proxy injecting AAAA responses. A single proxied wildcard record can silently break your entire setup by giving IPv6-preferring clients an alternate path that bypasses your local DNS.

## 🎉 Outcome

With Tailscale deployed, I now have:

1. **Full remote access** — all my web services are accessible from anywhere via their `*.arcade-lab.io` subdomains, exactly as they are on the LAN.
2. **Remote ad blocking** — Pi-hole filters DNS queries from my Tailscale devices, so I get network-wide ad blocking even when I'm away from home.
3. **Remote file access** — Samba shares work over Tailscale, so I can access my movies, photos, and files from anywhere.
4. **SSH from anywhere** — I can SSH into the server over Tailscale without exposing the SSH port to the public internet.
5. **Zero router configuration** — no port forwarding, no dynamic DNS, no exposed ports. Tailscale handles NAT traversal automatically.
6. **End-to-end encryption** — all traffic between my devices is encrypted with WireGuard, even on untrusted networks.

The best part: installing Tailscale on a new device and joining my tailnet takes about 30 seconds. No VPN profiles to configure, no certificates to install, no client configuration files to manage. Noice! 🎉
